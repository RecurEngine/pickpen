// 首次使用引导：主动把「登录 → 绑定仓库」这条路径摆到用户面前。两个触发点：
// ① 宿主在用户点击启用那一刻回调的 onUserEnable（冷启动不会触发）；
// ② 每次打开 Obsidian 的启动提醒（配置未完成时延迟弹出，见 index.ts scheduleStartupSetupReminder）。
// 两者共用同一个弹窗类与阶段判定，且都不落持久化标记：是否打扰由当前配置状态决定。
